...
def main():
    ...
    while not exit:
        ...
        if illuminate_white:
            empty = ~(game.get_b_board() | game.get_w_board())
            dragger.illuminate_pseudo_moves(canvas, game.get_w_bitboard_object(), empty, game.get_b_board(), game.history)
        if illuminate_black:
            empty = ~(game.get_b_board() | game.get_w_board())
            dragger.illuminate_pseudo_moves(canvas, game.get_b_bitboard_object(), empty, game.get_w_board(), game.history)
        ...
        if use_ai and game.current_turn() == robot.color and robot.choosing == False:
            ...
            move, _ = robot.minimax(bcopy, True, 2, hcopy, float("-inf"), float("inf"))
            if not move:
                print("GGILOSE")
            ...
            if game.get_proper_board(init_idx) == 0:
                print(BitBoard.print_board_2(game.get_b_board() | game.get_w_board()))
        ...
        for event in p.event.get():
            ...
            # Dragging motion
                ...
                if dragger.is_dragging:
                    game.show_bg()
                    # print(f"current board:\n{BitBoard.print_board_2(game.get_current_player_board() | game.get_opponent_board())}")
                    dragger.illuminate_moves(game.surface, game.get_current_player_board(), game.get_opponent_board(), game.history, game)
                    game.load(IMAGES)
                    dragger.update_mouse(p.mouse.get_pos())
                    dragger.update_blit(game.surface)
...
profile.run('main()', filename="profile.prof")
