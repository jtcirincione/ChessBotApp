...
class Dragger2:
    ...
    def drag2(self, bitboard: BitBoard, move: Move2, board_to_clear: BitBoard=np.uint64):
        if not bitboard:
            print(f"invalid move: {move.get_initial_idx(), move.get_final_idx()}")
            print(f"{type(bitboard)}")
    ...
    def illuminate_pseudo_moves(self, surface, white_boards: list[BitBoard], empty: np.uint64, opp_board: np.uint64, move_history):
        moves = []
        i = 0
        for board in white_boards:
            moves.extend(board.generate_moves(empty, opp_board, move_history))
            i += 1
        print(f"{len(moves)} possible moves.")
        for move in moves:
            idx = move.get_final_idx()
            row, col = Move2.convert_idx_to_rc(idx)
            rect = (col * SQ_SIZE, row * SQ_SIZE, SQ_SIZE, SQ_SIZE)
